...
//        makeNewGridMenu();
    ...
    {
        JMenuItem item = new JMenuItem();
        item.setFont(new Font("SansSerif", Font.PLAIN, 7));
//        BotBrain rb = (BotBrain)c.newInstance();
        ...
        try 
        {
            ...
            {
                ((RatBotWorld)world).addToAllRats((BotBrain) o);
...
//            public void actionPerformed(ActionEvent e)
//            {
//                setPlayMode(RatBotsArena.CHALLENGE_4);
//            }
//        }));
...
//                aw.clearAllObjectsFromGrid();
        ...
        menu.add(makeMenuItem("menu.arena.reset", new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                ((RatBotWorld)world).initializeMatch();
                display.repaint();
            }
        }));
...
//        }));
        rbMenu.add(makeMenuItem("menu.rats.clear", new ActionListener()
        {
            ...
            {
                control.stop();
                ((RatBotWorld)world).clearAllRats();
                repaint();
            }
        }));
...
//        }));
        ...
        menu.add(makeMenuItem("menu.tourney.begin", new ActionListener()
        {
            public void actionPerformed(ActionEvent e)
            {
                ((RatBotWorld)world).startRoundRobin();
                control.run();
...
//        { setPlayMode(RatBotsArena.CHALLENGE_4); return true; }
...
//        { setPlayMode(RatBotsArena.CHALLENGE_5); return true; }
        ...
        { ((RatBotWorld)world).addFromAllRats(0); return true; }
        if(description.equals("2"))
        { ((RatBotWorld)world).addFromAllRats(1); return true; }
        if(description.equals("3"))
        { ((RatBotWorld)world).addFromAllRats(2); return true; }
        if(description.equals("4"))
        { ((RatBotWorld)world).addFromAllRats(3); return true; }
        if(description.equals("5"))
        { ((RatBotWorld)world).addFromAllRats(4); return true; }
        if(description.equals("6"))
        { ((RatBotWorld)world).addFromAllRats(5); return true; }
        if(description.equals("7"))
        { ((RatBotWorld)world).addFromAllRats(6); return true; }
        if(description.equals("8"))
        { ((RatBotWorld)world).addFromAllRats(7); return true; }
        
        if(description.equals("T") || description.equals("t"))
        { Tail.toggleDURATION(); return true; }
        if(description.equals("R") || description.equals("r"))
        {     
            ((RatBotWorld)world).resetRatsInMaze();
            ((RatBotWorld)world).initializeMatch();
            display.repaint();
            return true; 
        }
