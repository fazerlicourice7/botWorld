 ...
 * @author 17noort
 */
public class BestBot_Tamim extends BotBrain {
    ...
    public double distanceTo(Location goal, int dir) {
        ...
        double dy = 0;
        if (dir == 0) {
            dx = Math.abs(goal.getRow() - (row - 1));
            dy = Math.abs(goal.getCol() - column);
        }
        if (dir == 90) {
            dx = Math.abs(goal.getRow() - row);
            dy = Math.abs(goal.getCol() - (column + 1));
        }
        if (dir == 180) {
    ...
    public boolean canMove2(int directionChoice, int row, int col) {

        int direction2 = directionChoice % 1000;
        Location myLoc = new Location(row, col);
        Location next2 = myLoc.getAdjacentLocation(direction2);
        if (!next2.isValidLocation()) {
            return false;
        }

        GameObject onNext = arena[next2.getRow()][next2.getCol()];
    ...
    public Location getEndLocationForDarting(int dir) {
        arena = getArena();
        Location end = new Location(10, 10);
        if (dir == 0) {
            if (canMove(dir)) {
                for (int row2 = getRow(); canMove2(dir, row2 - 1, column); row2--) {
                    end = new Location(row2 - 2, column);
                }
            } else{
                end = new Location(row,column);
            }
        } else if (dir == 90) {
            if(canMove(dir)){
            for (int col2 = getCol(); canMove2(dir, row, col2 + 1); col2++) {
                end = new Location(row, col2 + 2);
            }
            } else{
                end = new Location(row,column);
            }
        ...
        //System.out.println("Final Dart Location: " + end);

        return end;
    }

    public double distanceFromFinalDartPoint(Location dart, Location goal, int dir) {
        double dx;
        double dy;
        dart = getEndLocationForDarting(dir);

        dx = Math.abs(dart.getRow() - goal.getRow());
        dy = Math.abs(dart.getCol() - goal.getCol());
    ...
    public int chooseBestDirectiontoLocation(Location goal) {
        ...
        for (int dir = 0; dir < 360; dir += 90) { //loops through all possible directions
            ...
            score += ifInWalls(dir);
            score += getTimesBeenThere(dir);
            //System.out.println("TimesBeenThere: " + getTimesBeenThere(dir));
            score += distanceTo(goal, dir);
            //System.out.println("PathFinding DistanceTo: " + distanceTo(goal,dir));
            //System.out.println("CurrentLocation: " + row + ", " + column);
            //System.out.println("PathfindingScore towards " + dir + ": " + score);
            score2 += distanceFromFinalDartPoint(getEndLocationForDarting(dir), goal, dir);
            score2 += getTimesBeenThere(dir);
            //System.out.println("DartScore towards " + dir + ": " + score2);
            if (score < lowestScore) {
                lowestScore = score;
                bestPathDir = dir;
            }
            if (score2 < lowestScore2) {
                lowestScore2 = score2;
                bestDartDir = dir + 1000;
            }
            //System.out.println("bestDartDir: " + bestDartDir + " ,bestPathDir: " + bestPathDir);
            //System.out.println("----------------------------------------------" + "\n");

        }

        if (lowestScore2 < lowestScore && !inWalls()) {
            bestDir = bestDartDir;
        } else {
            bestDir = bestPathDir;
        }
...
//                        }
    ...
    public boolean inWalls() {
        ...
        if (!isThereGoldPrize()) {
            if ((row <= 11 && row >= 9 && (((column <= 3 && column >= 0)) || (column <= 20 && column >= 17))) || (column <= 11 && column >= 9 && (((row <= 3 && row >= 0)) || (row <= 20 && row >= 17)))) {
                result = true;
            }
        }
        return result;
